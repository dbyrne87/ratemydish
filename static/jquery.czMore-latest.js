            ...
            function loadMinus(recordset) {
                ...
                if(!options.styleOverride) {
                  btnMinus.css({
                      'float': 'right',
                      'border': '0px',
                      'background-image': 'url("/static/media/remove.png")',
                      'background-position': 'center center',
                      'background-repeat': 'no-repeat',
                      'height': '25px',
                      'width': '25px',
...
(function ($, undefined) {
    ...
    $.fn.czMoretwo = function (options) {
        ...
        return this.each(function () {
            var obj = $(this);
            var i = recordsetCount();
            var divPlus = '<div id="btnPlustwo" class="btnPlustwo"/>';
            var count = '<input id="' + this.id + options.countFieldPrefix + '" name="' + this.id + options.countFieldPrefix + '" type="hidden" value="0" size="5" />';

            obj.before(count);
            var recordset = obj.children("#second");
            obj.after(divPlus);
            var set = recordset.children(".recordsettwo").children().first();
            var btnPlustwo = obj.siblings("#btnPlustwo");
            ...
            if (recordset.length) {
                obj.siblings("#btnPlustwo").click(function () {
                    if (isMaxRecordset()){
                        return false;
                    }
                    var i = recordsetCount();
            ...
            function loadMinus(recordset) {
                var divMinus = '<div id="btnMinustwo" class="btnMinustwo" />';
                $(recordset).children().first().before(divMinus);
                var btnMinustwo = $(recordset).children("#btnMinustwo");
                if(!options.styleOverride) {
                  btnMinustwo.css({
                      'float': 'right',
                      'border': '0px',
            ...
            function minusClick(recordset) {
                $(recordset).children("#btnMinustwo").click(function () {
                    var i = recordsetCount();
                    var id = $(recordset).attr("data-id")
                    $(recordset).remove();
                    resetNumbering();
